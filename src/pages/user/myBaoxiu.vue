<template>
	<view :class="uiStyle=='normal'?'':'elder'">
		...
		<u-empty :text="current==0?'暂无报修，点击下方新增报修':'暂无数据'" mode="data" v-if="data.length == 0" margin-top="100"></u-empty>

		<view class="bottom">
			<button @click="toAdd()">新增报修</button>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			getData() {
				this.http.post('repair/myRepair', {
					status:this.current
				}, 2).then((data) => {
					this.data = data.list
				})
			...
			toDetail(id) {
				uni.navigateTo({
					url: 'baoxiuDetail?id=' + id +'&&type=' + this.current
				})
			},
			toAdd() {
				uni.navigateTo({
