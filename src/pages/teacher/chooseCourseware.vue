				...
				<view class="card card2" v-for="(item,index) in courseList" @click="choosen(item)">
					...
					<view class="content" style="justify-content: flex-start;">
						...
						<view>
							...
							<view class="content" style="justify-content: flex-start;">
								<view>
									<view style="display: flex; margin-top: 18upx;flex-wrap: wrap;">
										<view class="tag">{{item.source_name}}</view>
										<view class="tag">{{item.age_section_name}}</view>
										<view class="tag">{{item.language_name}}</view>
