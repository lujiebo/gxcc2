<template>
	<view>
		<view class="content" v-if="type == 0">
			<view class="icon">
				<uni-icons type="checkbox-filled" size="84" color="green"></uni-icons>
			</view>
			<view class="title">预约成功！</view>
			<view><span style="color: #FEA72B;">{{time}}</span>秒后自动跳转到预约详情页面</view>
			<button class="btn-l" style="width: 90%;" @click="click()">立即前往</button>
		</view>
		<view class="content" v-if="type == 1">
			<view class="icon">
				<uni-icons type="clear" size="84" color="red"></uni-icons>
			</view>
...
<script>
	export default {
		...
		methods: {
			click() {
				uni.redirectTo({
					url: '../user/yuyueDetail?id=' + this.id
				})
			},
			clickRe() {
				uni.navigateBack({
